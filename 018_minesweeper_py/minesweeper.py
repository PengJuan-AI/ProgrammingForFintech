...
def make_board(width, height, num_mines):
    ...
    """
    board = [[SQUARE_UNKNOWN for i in range(0,width)] for j in range(0,height)]

    board_has_mines = board
    while num_mines:
        board_has_mines = add_random_mine(board_has_mines)
        num_mines -= 1

    return board_has_mines
...
def count_mines(board, x, y):
    ...
    for nx in range(x-1,x+2):
        ...
        for ny in range(y-1, y+2):
            if ny<0 or ny>=board_height(board) or (nx==x and  ny==y):
                continue
...
def check_win(board):
    ...
    for i in range(board_height(board)):
        if not SQUARE_UNKNOWN in board[i]: win += 1
    
    if win == board_height(board): return True
    else: return False
